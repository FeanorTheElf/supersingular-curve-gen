...
\end{problem}
An algorithm solving the above problem can then easily made into a collision-free hash function by prepending it with a secure hash function.
...
\paragraph{CM methods} Using the theory of complex multiplication of curves over $\C$ and their reduction modulo primes, Brökner \cite{constructing_supersingular_curves} found an algorithm that can find supersingular curves $E$ over $\F_p$, even for astronomically large prime $p$.
...
This is clearly a weakness, as small endomorphism rings can be computed efficiently, for example by an exhaustive search for small endomorphisms.
...
\begin{equation*}
    x^2 + D y^2 = m p
\end{equation*}
where $\End(E) \cong \Z[\sqrt{D}]$, i.e. $d(\End(E)) = 4D$.
Note that we want a lower bound (we have an upper bound by the degree argument), and so we can ignore the possibilities in which $\End(E)$ is not of this form (i.e. $\End(E) \cong \Z[(1 + \sqrt{D})/2]$).
...
\begin{align*}
    &\#\{ j \in \bar{\F}_p \ | \ \Phi_m(j, j^p) = 0 \} \geq \sum_{\substack{\text{$x^2 - Dy^2 = mp$ solvable}\\\text{with $x \perp y$}}} h(4D) \\
    \geq& \sum_{0 < x < \sqrt{mp}} h(4m p - 4x^2) \approx \sum_{0 < x < \sqrt{mp}} 2\sqrt{mp - x^2} \\
...
\end{prop}
\begin{proof}
    Assume that $\O$ is an imaginary quadratic order with $p \notdivides d(\O)$ that has such an element $\beta \in \O \setminus \Z$ of norm $n$.
...
While the Diophantine equation $x^2 + D y^2 = m$ has been thoroughly studied (see e.g. \cite{cox_primes_of_form}), the best characterization for it being solvable (assuming $m = p$ is prime) involves the so-called Hilbert class polynomial
\begin{equation*}
...
\begin{proof}
    ...
    For any ordinary curve $E$, denote now by $E_R$ the root of the lava flow tree containing $E$ in the $l$-isogeny volcano.
    ...
    Thus $\l_1^{2f - 4i}$ is principal, and its generator gives a nontrivial endomorphism $\phi$ of $\End(E_R)$ of degree $l^{2f}$.
    Now
    \begin{equation*}
        E = E_0 \to E_1 \to ... \to E_i = E_R \ \overset{\phi}{\longrightarrow} E_R = E_i \to E_{i - 1} \to ... \to E
    \end{equation*}
    gives a nontrivial endomorphism of $E$ with degree $l^{2f}$, so we are done.
...
Furthermore an algorithm that is able to efficiently compute a random root of $F_{p, l^f, l^e}$ can be used to generate a random supersingular curve with very high probability.
...
\begin{example}
    ...
    \begin{figure}
        \begin{minipage}{0.5\textwidth}
            \centering
            \includegraphics[width = \textwidth]{../example_51_ordinary.png}
        \end{minipage}%
        \begin{minipage}{0.5\textwidth}
            \centering
            \includegraphics[width = 0.6\textwidth]{../example_51_supersingular.png}
        \end{minipage}
...
\begin{prop}
    ...
    Then
    \begin{equation*}
        \Bigl\lfloor \frac p {m^2} \Bigr\rfloor \leq n \leq \Bigl\lfloor \frac {16p^2} {m^3} \Bigr\rfloor
    \end{equation*}
    Furthermore, consider the number $N$ of ordinary j-invariants $j \in \F_{p^2}$ such that $\pi \in \Z + m\End(j)$.
    Under GRH, we have then for $m \leq p^2$ that
    \begin{equation*}
...
\begin{proof}
    ...
    \end{equation*}
    Thus the imaginary quadratic order $\O$ with discriminant $D$ contains a nontrivial element of norm $p^2$, which must be the Frobenius $\pi$ (or its conjugate).
...
Note that also the computation of the elimination ideal from \ref{eq:elimination_ideal} is not trivial.

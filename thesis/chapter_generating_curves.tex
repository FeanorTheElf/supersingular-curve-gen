...
\begin{proof}
    ...
    \end{equation*}
    and so $\O/\mathfrak{f} \cong \Z/f\Z$, hence $\#(\O/\mathfrak{f})^* = \phi(f)$.
    ...
    \begin{align*}
        ...
        \end{cases} \\\
        =& \begin{cases}
            p^{2e} - p^{2e - 2} & \text{if $\left(\frac {-d_K} p \right) = -1$} \\
            f\phi(f) & \text{if $\left(\frac {-d_K} p \right) = 0$} \\
            \phi(f)^2 & \text{if $\left(\frac {-d_K} p \right) = 1$}
        \end{cases}
    \end{align*}

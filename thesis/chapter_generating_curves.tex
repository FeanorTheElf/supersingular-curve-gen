...
In our research, we mainly focused on analysing and improving the second proposal in \cite{base_paper}, which was proposed by Katherine Stange.
...
Furthermore, at the moment we content ourself with a very crude estimate, but note that everything can be made rigorous.
...
\begin{align*}
    ...
    \geq& \sum_{0 < x < \sqrt{mp}} h(4mp - 4x^2) \approx \sum_{0 < x < \sqrt{mp}} 2\sqrt{mp - x^2} \\
    \approx& \ 2\int_0^{\sqrt{mp}} \sqrt{mp - x^2} dx = 2mp \int_0^1 \sqrt{1 - x^2} dx \in \Omega(mp)
\end{align*}
...
\begin{prop}
    ...
    Under GRH, we then for $m \leq p^2$ that
    \begin{equation*}
...
\subsection{A more explicit representaton}

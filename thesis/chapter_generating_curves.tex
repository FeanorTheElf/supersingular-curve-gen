...
\begin{itemize}
    \item How large is the fraction of supersingular roots of $\Phi_n(X, X^p)$ resp. of $f_{p, n, m}$?
    And how large is the fraction if we only consider roots over $\F_{p^2}$?

    In particular, for the method to have any hope of success, we require the fraction to be at least $1/\mathrm{poly}(\log(p))$.
    
    \item How can we efficiently compute $f_{p, n, m}$, especially since $p$ and possibly $n$ resp. $m$ are exponentially large?
...
Finally, we present our first main result, which answers the first open question for a subset of the $m$ and $n = l^e$.
Our approaches for an efficient computation also apply to this special case.
However, we are not able to completely answer the second question, and thus cannot solve the main problem.

In the case that both $n$ and $m$ are small (but $p$ is exponential), \cite{base_paper} presents an approach that can find roots of $f_{p, n, m}$ even though $p$ is exponentially large, so $\Phi_n(X, X^p)$ and $\Phi_n(X, X^p)$ have exponential degree.
...
Let now $\O_0$ be the order with conductor $[\O_K : \O] / l^e$, where $e$ is maximal.
Now Corollary~\ref{prop:inclusion_orders_surjection} shows that we can find a sequence of ideals $(\a_i)_i$ with $\a_i \leq \Z + l^i\O_0$ and $[\a_{i + 1}] \mapsto [\a_i]$ (under the surjection of Corollary~\ref{prop:inclusion_orders_surjection}) such that $\a_e = \a$.

Now this gives us a graph automorphism of the $l$-isogeny subgraph induced by the set of curves 
\begin{equation*}
    \Ell^+(\O_0) := \{ \text{$E$ elliptic curve} \ | \ \text{$\End(E) \cong \Z + l^i\O_0$ for some $i$} \}
\end{equation*}
given by
\begin{equation*}
    \Ell^+(\O_0) \to \Ell^+(\O_0), \quad E \mapsto [\a_i].E
\end{equation*}
...
\begin{proof}
    ...
    \begin{figure}
        ...
        \caption{\label{fig:split_case} 
            ...
            Since $E_R$ is defined over $\F_{p^2}$, we know that $[(p, \pi)]^2.E_R = E_R$ and so $E_R^{(p)}$ is on the opposite side of the vulcano.

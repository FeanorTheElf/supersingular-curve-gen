...
In the ordinary case, the class group action might be used to compute neighbors in the $l$-isogeny graph, provided we know the endomorphism ring of the start curve.
However, finding the endomorphism ring is a hard problem in itself, and thus this method is not really practical.
Furthermore, we currently have no useful approach to compute isogeneous curves in the supersingular setting.

One solution to this problem is given by modular curves.
In particular, the existence of a nontrivial $l$-isogeny between curves is an algebraically closed condition, i.e. can be described by a polynomial.
In this section, we will study on how to do this.

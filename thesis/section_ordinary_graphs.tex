...
\begin{lemma}
    \label{prop:class_group_map_surjective}
    The map
    \begin{equation*}
        \Cl(\O) \to \Cl(\O_K)
    \end{equation*}
    ...
    \end{equation*}
    and Lemma~\ref{prop:invertible_ideal_characterization} is surjective.
\end{lemma}
\begin{proof}
    Consider the set $S$ of primes $\p \leq \O$ such that $\Norm(\p) \not\perp d(\O)$.
    Our proof will explicitly consider what happens at a prime $\q \leq \O_K$ over a prime $\p \in O$.
    We then use that there are only finitely many such primes, and construct a preimage locally at those.
    Finally, we remark that at all other primes, the claim is trivial, as $\O$ and $\O_K$ look the same at $\q$ (see Prop.~\ref{prop:coprime_ideals_order}).

    For a prime $\p \in S$, let now $\q_1, ..., \q_r$ be the primes over $\p$ in $\O_K$.
    Then there is
    \begin{equation*}
        \beta_i^{(\p)} \in \q_i \setminus \Bigl( \q_i^2 \cup \bigcup_{\q} \q \Bigr)
    \end{equation*}
    where $\q$ runs over all finitely many primes of $\O_K$ which lie over some prime in $S$.
    This is possible, since $(\O_K)_{\q_i}$ is a PID.

    Now we have that
    \begin{equation*}
        v_{\q_i}(\beta_i) = 1 \quad \text{and} \quad v_{\q}(\beta_i) = 0
    \end{equation*}
    Hence, for any fractional, invertible ideal $\a$ of $\O'$ we can multiply it by the $\beta_i^{(\p)}$ to achieve that
    \begin{equation*}
        v_{\q_i}(\a) = v_{\q_j}(\a) \quad \text{whenever $\q_i$ and $\q_j$ lie over the same prime $\p \in S$}
    \end{equation*}
    Furthermore, we can achieve that
    \begin{equation*}
        e_{\q} \divides v_{\q}(\a) \quad \text{where $e_\q$ is the ramification degree of $\q$ over $\Q$}
    \end{equation*}
    Note that this is trivial for all primes $\q$ except those over a prime in $S$, as the others have norm coprime to $d(\O)$, so coprime to $d(\O_K)$ and are thus unramified, i.e. $e_\q = 1$.

    Now by Lemma~\ref{prop:invertible_ideal_characterization}, we find an invertible, fractional ideal $\b$ of $\O$ with
    \begin{equation*}
        \b\O_\p = p^{v_\q(\a)/e_\q}\O_\p \quad \text{where $\q$ is a prime of $\O_K$ over $\p \in S$, over a prime number $p$}
    \end{equation*}
    and (by Lemma~\ref{prop:prime_coprime_localization_same})
    \begin{equation*}
        \b\O_\p = \a(\O_K)_\p \quad \text{where $\q$ is a prime of $\O_K$ over a prime $\p \notin S$}
    \end{equation*}
    Now we clearly have
    \begin{equation*}
        \b(\O_K)_\q = \a(\O_K)_\q
    \end{equation*}
    for all primes $\q$ of $\O_K$, thus $[\b]$ maps to $[\a]$ under $\Cl(\O) \to \Cl(\O_K)$.
\end{proof}
...
\begin{proof}
    ...
    \begin{equation*}
        g: \Cl(\O) \to \Cl(\O_K)
    \end{equation*}
    in Lemma~\ref{prop:class_group_map_surjective}.
...
\begin{proof}
    ...
    \end{equation*}
    This isomorphism follows from the Chinese remainder theorem (Lemma~\ref{prop:chinese_remainder_theorem_variant}) and the fact that the prime ideals $\q$ over $\p\O_K$ give all prime ideals of $R_\p$.
...
\end{proof}
We remark that the crater of a volcano is a single vertex without a loop, if $(l)$ is inert in $\O$.

...
We can for example derive a result for the inclusion of two orders.
\begin{corollary}
    \label{prop:inclusion_orders_surjection}
    Consider an inclusion of orders $\O \subseteq \O'$ in $K$.
    Then there is a surjective morphism of groups
    \begin{equation*}
        \Cl(\O) \to \Cl(\O')
    \end{equation*}
\end{corollary}
\begin{proof}
    Using Lemma~\ref{prop:invertible_ideal_characterization}, we see that the natural surjective morphism
    \begin{equation*}
        \bigoplus_{\p \leq \O} K^*/\O_\p^* \to \bigoplus_{\q \leq \O'} K^*/(\O_\q')^*, \quad (a_\p)_\p \mapsto (\a_{\q \cap \O})_\q
    \end{equation*}
    induces a map as wanted.
\end{proof}
...
\begin{lemma}
    ...
    The function
    \begin{equation*}
        f \mapsto \frac {\#(\O_K/f\O_K)^*} {\#((\Z + f\O_K)/f\O_K)^*}
    \end{equation*}
    is multiplicative.
\end{lemma}
\begin{proof}
    We have that $\#((\Z + f\O_K)/f\O_K)^* = \#(\Z/f\Z)^* = \phi(f)$ is multiplicative.
...
\begin{prop}
    ...
    Let $\O_0$ be an orders in a quadratic imaginary number field $K$ with $\O_K = \{ \pm 1 \}$.

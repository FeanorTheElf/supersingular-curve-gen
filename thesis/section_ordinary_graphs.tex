...
\begin{proof}
    ...
    For a given prime $\p$ of $\O$, we can ensure by multiplication of elements in $K^*$ that the valuations $v_{\q}(\a) = v_{\q'}(\a)$ are equal, where $\q, \q'$ are the two primes of $\O_K$ over $\p$ (this makes sense, as $(\O_K)_\q$ is a discrete valuation ring).

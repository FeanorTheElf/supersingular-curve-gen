...
\section{More ideas on computing the endomorphism ring}
Let $K = \Z[\pi] \otimes \mathbb{Q}$ be the number field, and suppose it has discriminant $D$.
Then 1 and $\alpha := \frac {D + \sqrt{D}} 2$ form an integral basis of $\O_K$.
Now note that
\begin{equation*}
    \beta := (a + \alpha)(b + c\alpha) = a b - c \underbrace{\frac {D^2 - D} 4}_{=: \delta} + (ac + b + cD)\alpha
\end{equation*}
If we want this to be a generator of $\Z[\pi]$, we set $ac + b + cD = f$ where $f = [\O_K : \Z[\pi]]$.
In other words, have $b = f - c(a + D)$.
Thus find
\begin{equation*}
...
\begin{equation*}
    | af - a^2c - acD - c\delta |, \quad a, c \in \Z
\end{equation*}
Note that once we found a suitable $\beta$, have that $\End(E) = \Z + [\O_K : \End(E)] \beta \Z$, so we just have to find the maximal $n$ such that $n \divides \beta$ in $\End(E)$.
Note that $\beta = \pi + m$ for some integer $m$, and so we can do this by computing the kernel of that endomorphism.
This is easier the smaller the prime powers dividing $N(\beta)$ are, so we want $N(\beta)$ to be as small as possible.

...
\section{The local-to-global principle for the endomorphism ring}
...
\end{proof}
\begin{lemma}
    \label{prop:prime_generators_in_order}
...
\end{proof}
\begin{lemma}
    \label{prop:prime_splitting_order}
    Let $p$ be a prime number that does not divide $[\O_\K : \O]$.
    Assume $\O = \Z[\phi]$.
    \begin{itemize}
        \item Suppose that $p$ splits in $\O_\K$ as $p\O = \p_1 \p_2$ (this includes the case that $p$ is ramified, i.e. $\p_1 = \p_2$).
        Then have in $\O$ that
        \begin{equation*}
            p\O = (\p_1 \cap \O)(\p_2 \cap \O)
        \end{equation*}
        These are all prime ideals over $p$, and so in particular the decomposition is unique.
        \item Suppose that $p$ is inert (in $\O_\K$), i.e. $p\O_\K$ is prime. 
        Then clearly also $p\O$ is prime, and it is the only prime ideal over $p$.
    \end{itemize}
    In particular, all primes over $p$ are invertible in $\O$.
\end{lemma}
\begin{proof}
    We have that $\mathrm{MiPo}(\phi) = f_1(X) f_2(X) \mod (p)$, and so
    \begin{equation*}
        p\O_\K = \underbrace{(p, f_1(\phi))}_{= \p_1}\underbrace{(p, f_2(\phi))}_{= \p_2}
    \end{equation*}
    in $\O_\K$.
    Since $p, f_1(\phi), f_2(\phi) \in \O$, we see that
    \begin{equation*}
        p \in (\p_1 \cap \O)(\p_2 \cap \O)
    \end{equation*}
    On the other hand, have $f_1(\phi)f_2(\phi) = 0 \mod (p)$, so also
    \begin{equation*}
        p\O \supseteq (\p_1 \cap \O)(\p_2 \cap \O)
    \end{equation*}
    Now assume we have a prime ideal $\q$ over $p$.
    Then clearly $p \in \q$.
    In particular, have also $f_1(\phi)f_2(\phi) \in \q$ since $p \divides f_1(\phi) f_2(\phi)$.
    wlog $f_1(\phi) \in \q$. Thus $\p_1 \subseteq \q$.
    Since $\O$ has Krull dimension 1 \Cite[I.12.2]{neukirch}, it follows that $\q = \p_1$.

    It is clear that $p\O$ is prime.
    Assume there is a prime ideal $\q \leq \O$ over $p$, i.e. $\q \cap \Z = (p)$.
    So $p \in \q$ and thus $p\O \subseteq \q$.
    Again, since $\O$ has Krull dimension 1, we see that $\q = p\O$.
\end{proof}
\begin{prop}
    Let $\mathfrak{I}(\O)$ resp. $\mathfrak{I}(\O_\K)$ denote the set of invertible ideals of norm $\perp [\O_\K : \O]$.
    Then
    \begin{align*}
        \mathfrak{I}(\O) &\to \mathfrak{I}(\O_\K) \\
        \a &\mapsto \a\O_\K
    \end{align*}
...
\begin{proof}
    ...
    Assume $\a \leq \O_\K$ with $\a = \prod \p_i^{e_i}$ where $\p_i$ is a prime over $p_i$.
    By Lemma~\ref{prop:prime_generators_in_order}, we see that
    \begin{equation*}
        (\a \cap \O) = \prod (\p_i \cap \O)
    \end{equation*}
    This shows that $\a \cap \O$ is invertible, as each $\p_i \cap \O$ is by Lemma~\ref{prop:prime_splitting_order}.
    Furthermore, this shows that $\a$ has a set of generators in $\O$, and thus $(\a \cap \O)\O_\K = \a$.

    Finally, consider an ideal $\a \leq \O$.
    By ideal factorization, have again $\a\O_\K = \prod \p_i$.
    As above, we find
    \begin{equation*}
        (\a\O_\K \cap \O) = \prod (\p_i \cap \O)
    \end{equation*}
    Since 
\end{proof}
\begin{prop}
